...
describe("operation failure policy", () => {
    it("only recognized validation codes establish invalidity", () => {
        ...
        expect(
            classifyOperationFailure(new Error("execution reverted"))
        ).toMatchObject({ kind: "upstream_error", retryable: true })
        expect(
            classifyOperationFailure({
    ...
    it("transport causes override an outer validation error", () => {
        ...
        expect(classifyOperationFailure(error)).toMatchObject({
    ...
    it("never publishes raw provider URLs or signed payloads", () => {
        ...
        const reason = publicOperationReason(
            `AA23 reverted https://rpc.test/secret signature: 0x${payload}`
        )
        ...
        expect(publicOperationReason(`userOp ${hash} reverted`)).toContain(hash)
        expect(
            userOperationStatusSchema.parse({
                status: "rejected",
